...
export function executarAuditoriaDinamica(
  ...
  if (valorConsulta === 0 && valorSadt === 0) {
    const isSadtOnly = conteudo.includes('guiaSP-SADT') || conteudo.includes('procedimentoExame') || conteudo.includes('valorTotalGeral');
    if (isSadtOnly) {
      ...
      if (isSurgery && !/<(?:[^:]+:)?viaAcesso/i.test(bloco)) {
        ...
        if (originalMatch) {
          inconsistencias.push({
            ...
            linha: lineTarget,
            tipo: "Erro Regra",
            desc: `${prefix}Linha ${lineTarget}: Tag nao preenchida.: _viaAcesso_ para o Procedimento Cirúrgico: _${cod}_.`,
